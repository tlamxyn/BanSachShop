...
class HomeController extends Controller
{
    ...
    public function search(Request $request)
    {
        if ($request->search_content!=null) {
            ...
            $danhsach_sach = DB::table('sach')->where('Ten', 'LIKE', '%'.$request->search_content.'%')
                                            ->orWhere('TacGia', 'LIKE', '%'.$request->search_content.'%')->get();

            $thongbao = "Không tìm thấy";
            return view('Users.Search.index', compact('danhsach_sach', 'thongbao'));
        } else {
